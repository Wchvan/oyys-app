<template>
  <view class="w-full h-full">
    <view v-for="(item, index) in userStore.favoriteSetList" :key="index" class="order-card">
      ...
      <view class="card-item">
        <view class="sale" :class="getStatus(item) !== '今日在售' ? 'sale-disable' : ''">{{ getStatus(item) }}</view>
        <button class="btn" @click="handleLike(index, item.isLiked)">{{ item.isLiked ? "取消收藏" : '收藏' }}</button>
      </view>
    </view>
  </view>
</template>
